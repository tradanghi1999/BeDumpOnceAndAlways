...
namespace IT
{
    public partial class V_IT_Vong1_ChonDoi : View
    {
        ...
        {
            if(_ctrl is IT_Vong1_ChonDoi_Controller)
            {
                (_ctrl as IT_Vong1_ChonDoi_Controller).GetDoiThiVong1();
            }
        ...
        private void BtnChiTietDiemDoi1_Click(object sender, EventArgs e)
        {
            ...
            //
            GetReady(sender as Button);
            btnVong1_Doi1.Enabled = true;
        }
        ...
        private void BtnChiTietDiemDoi3_Click(object sender, EventArgs e)
        {
            //throw new NotImplementedException();
            GetReady(sender as Button);
            btnVong1_Doi3.Enabled = true;
        }

        private void GetReady(Button btn)
        {
            try
            {
                (_ctrl as IT_Vong1_ChonDoi_Controller).MakeTeamsPending();
                btn.Enabled = false;
            }
            catch(Exception e)
            {
        ...
        private void V_IT_Vong1_ChonDoi_Load(object sender, EventArgs e)
        {
            if (_ctrl is IT_Vong1_ChonDoi_Controller)
            {
                (_ctrl as IT_Vong1_ChonDoi_Controller).GetTeams();
            }
            //_ctrl.GetTeams();
